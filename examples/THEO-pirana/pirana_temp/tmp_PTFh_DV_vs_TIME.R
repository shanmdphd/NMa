...
for (i in 1:length(model_names)) {
    fname <- paste("pirana_reports/",names(models)[i],"_DV_vs_TIME.pdf", sep="")
...
# R code for plotting starts here
        pl <- xyplot (DV~TIME, data=tab,
                      main = paste (model_names[i],": ", mod$description, sep=""), cex=0.7,
                      pch=1, xlab="Time", ylab="Dependent variable")
########################################################################################################
    } else {
