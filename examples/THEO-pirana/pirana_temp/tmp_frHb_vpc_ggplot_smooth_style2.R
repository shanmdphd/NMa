...
fname <- paste("pirana_reports/vpc_", folder, "_smooth.pdf", sep="")
if (runno != "") {
  fname <- paste("pirana_reports/",runno,"_vpc_smooth.pdf", sep="")
}
if (!file.exists("pirana_reports")) {dir.create ("pirana_reports")}
if (file.exists (paste("pirana_reports/vpc_",folder,"_smooth.pdf", sep=""))){
    file.remove (paste("pirana_reports/vpc_",folder,"_smooth.pdf", sep=""))
}
...
# plot all layers
pl <- pl +
    geom_ribbon(data=vpc_dat,aes_string(x='(upper+lower)/2',ymin=paste0('`',ci,'.CI.for.',lo.pi,'.from`'),
                                                     ymax=paste0('`',ci,'.CI.for.',lo.pi,'.to`')), linetype=0, alpha=0.2, fill=ci.out.col) +
    geom_ribbon(data=vpc_dat, aes_string(x='(upper+lower)/2', ymin=paste0('`',ci,'.CI.for.',up.pi,'.from`'),
                                                     ymax=paste0('`',ci,'.CI.for.',up.pi,'.to`')), linetype=0, alpha=0.2, fill=ci.out.col) +
    geom_ribbon(data=vpc_dat, aes_string(x='(upper+lower)/2',ymin=paste0('`',ci,'.CI.for.50.from`'),
