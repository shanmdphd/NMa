...
for (i in 1:length(model_names)) {
    fname <- paste("pirana_reports/",names(models)[i],"_DV_vs_TIME_ind.pdf", sep="")
...
# R code for plotting starts here
        pl <- xyplot (DV~TIME | factor(ID), data=tab, type="b", cex=0.4,
                      main = paste (model_names[i],": ", mod$description, sep=""),
                      auto.key=T, as.table=T, pch=1,
