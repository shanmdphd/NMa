...
for (i in 1:length(model_names)) {
    fname <- paste("pirana_reports/",names(models)[i],"_DV_PRED_vs_TIME.pdf", sep="")
...
# R code for plotting starts here
        ...
        pl <- xyplot (DV+PRED~TIME, data=tab,
                      type = c("p","p"), distribute.type= TRUE,
                      cex=0.6, col=colours,
                      key = list(text = list(c("DV", "IPRED")),
                          lines=c(1,1), col = colours, pch = c(1), type = c("p", "p")),
                      as.table=T,
